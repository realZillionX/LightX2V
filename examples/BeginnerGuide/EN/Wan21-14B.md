...
`num_frames` means the frame count of the target video (for the wan2.1 model, fps=16, so num_frames=81 represents a video duration of 5 seconds)

`size` specifies the target dimensions as `[height, width]`.
...
`message dictionary` means the content of the request sent to the server. If `seed` is omitted or null, it defaults to 42. If `save_result_path` is omitted or null, no result file is saved.
