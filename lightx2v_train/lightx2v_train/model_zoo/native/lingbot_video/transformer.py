# Adapted from the LingBot-Video diffusers implementation under Apache-2.0.
import math
import os
...
class LingBotVideoRotaryEmbedding(nn.Module):
    ...
    def forward(self, position_ids: torch.Tensor) -> torch.Tensor:
        ...
        needs_rebuild = self.freqs_cis is None or any(max_value >= axis_length for max_value, axis_length in zip(max_vals, self.axes_lens))
        if needs_rebuild:
...
class LingBotVideoSparseMoeBlock(nn.Module):
    def __init__(self, hidden_size, intermediate_size, num_experts, top_k, moe_intermediate_size, score_func, norm_topk_prob, n_group, topk_group, routed_scaling_factor, n_shared_experts):
        super().__init__()
    ...
    def _reorder_tokens(tokens: torch.Tensor, top_scores: torch.Tensor, top_indices: torch.Tensor, num_experts: int):
        ...
        if backend not in {"sort", "argsort", "default"}:
            raise ValueError(f"Unsupported LINGBOT_MOE_REORDER_BACKEND={backend!r}; training only supports sort")
    ...
    def _pad_grouped_tokens_loop(tokens: torch.Tensor, counts: torch.Tensor, align: int = 8):
        ...
        for expert_idx in range(num_experts):
            ...
            permuted_indices[write_start : write_start + length] = torch.arange(start, start + length, device=tokens.device, dtype=torch.int64)
    ...
    def _run_selected_experts(
        ...
        raise ValueError(f"Unsupported LINGBOT_MOE_EXPERT_BACKEND={backend!r}; training only supports grouped_mm")
    ...
    def _restore_tokens(
        ...
        if backend not in {"scatter", "default"}:
            raise ValueError(f"Unsupported LINGBOT_MOE_RESTORE_BACKEND={backend!r}; expected scatter, chunked_scatter, weighted_scatter, or index_add")
...
class LingBotVideoBlock(nn.Module):
    ...
    def forward(
        ...
        if temb6.ndim != 2 or temb6.shape[0] != expected_tokens:
            raise ValueError(f"LingBotVideoBlock expects token-level temb6 with shape (B*S, 6D); got {tuple(temb6.shape)} for hidden states {tuple(x.shape)}.")
    ...
    def forward(
        ...
        for block in self.blocks:
            if torch.is_grad_enabled() and getattr(self, "gradient_checkpointing", False):
                ...
                def block_forward(joint_states, modulation, rotary_embedding, current_block=block):
                ...
                joint = self._gradient_checkpointing_func(block_forward, joint, temb6, rotary)
            else:
                joint = block(
                    joint,
                    temb6,
