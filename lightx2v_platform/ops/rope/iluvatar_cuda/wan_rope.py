...
class IluvatarWanRope(RopeTemplate):
    ...
    def apply(self, xq: torch.Tensor, xk: torch.Tensor, cos_sin_cache: torch.Tensor):
        """
        Apply WAN RoPE using triton operations, optimized for Iluvatar cuda.
        Args:
            xq: Query tensor
            xk: Key tensor
        ...
        """
        assert torch.is_complex(cos_sin_cache), "cos_sin_cache must be complex tensor"
        cos, sin = cos_sin_cache.real.contiguous(), cos_sin_cache.imag.contiguous()
        if xq.dim() == 3:
            xq = xq.unsqueeze(0)
            xk = xk.unsqueeze(0)
        xq = apply_rotary(xq, cos, sin, interleaved=True)
        xk = apply_rotary(xk, cos, sin, interleaved=True)
        if xq.dim() == 4:
            xq = xq.squeeze(0)
            xk = xk.squeeze(0)
...
def rotary_kernel(
    ...
    BLOCK_K: tl.constexpr = ROTARY_DIM
