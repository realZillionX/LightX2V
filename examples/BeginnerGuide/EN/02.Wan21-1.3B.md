...
`num_frames`: Number of frames in the target video (for wan2.1, fps=16, so num_frames=81 means a 5-second video).
...
The message dictionary is the content sent to the server. If `seed` is omitted or null, it defaults to 42. If `save_result_path` is omitted or null, no result file is saved.
