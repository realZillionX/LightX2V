...
1. `prompt`, `negative_prompt`, and `image_path` are basic inputs for video generation. `image_path` can be an empty string, indicating no image input is needed. `size` optional output video resolution, defaults to the configured resolution.
