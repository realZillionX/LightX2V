...
cls = next(node for node in ast.parse(source.read_text()).body if isinstance(node, ast.ClassDef) and node.name == "NeoppTransformerInfer")
cls.bases = []
cls.body = [node for node in cls.body if isinstance(node, ast.FunctionDef) and node.name in {"_fm_head", "_fm_head_pixel", "_patchify_pixels"}]
...
def test_u15_pixel_head_matches_conv_decoder_and_patchify():
    ...
    infer.patch_size = 16
    infer.merge_size = 2
    infer.scheduler = SimpleNamespace(image_prediction=torch.zeros(1, 3, token_h * output_patch_size, token_w * output_patch_size))
